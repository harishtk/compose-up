...
fun Modifier.angleGradientBackground(
    colors: List<Color>,
    degrees: Float,
) = drawBehind {
    /*
    ...
    val degreesNormalized = (degrees % 360).let { if (it < 0) it + 360 else it }
    val alpha = (degreesNormalized * PI / 180).toFloat()

    val gradientLength = when (alpha) {
        // ray from center cuts the right edge of the rectangle
        in 0f..gamma, in (2 * PI - gamma)..2 * PI -> { x / cos(alpha) }
        // ray from center cuts the top edge of the rectangle
        in gamma..(PI - gamma).toFloat() -> { y / sin(alpha) }
        // ray from center cuts the left edge of the rectangle
        in (PI - gamma)..(PI + gamma) -> { x / -cos(alpha) }
        // ray from center cuts the bottom edge of the rectangle
    ...
    drawRect(
        brush = Brush.linearGradient(
            colors = colors,
            // negative here so that 0 degrees if left -> right
